...
$link_image = ACF::getField('link-image', $data, 'no');
...
<section class="module hero uk-inline <?php echo esc_attr($type); ?> <?php echo esc_attr($video_class); ?> <?php echo esc_attr($overlay_class); ?>" id="<?php echo esc_attr($row_id); ?>">
    <?php if ($hero_media === 'image-hero') {
        if (!empty($image_id)) { ?> 
            ...
            <?php if ($type !== 'short-hero') {
                if ($link_image === 'yes' && !empty($link)) { ?>
                    <a href="<?php echo esc_url($link['url']); ?>" class="hero__image-link uk-position-cover" aria-label="<?php echo esc_attr($link['title']); ?>">
                <?php }
                    if (!empty($mobile_image_id)) { ?> 
                        <div class="hero__image-container-mobile uk-background-cover<?php echo $dynamic_swap_mobile;?>" style="background-image: url(<?php echo Media::getAttachmentSrcById($mobile_image_id, 'full'); ?>); <?php echo Util::getInlineBackgroundStyles($data); ?>" data-default-image="<?php echo Media::getAttachmentSrcById($mobile_image_id, 'full'); ?>"></div>
                    <?php } else { ?>
                        <div class="hero__image-container-mobile uk-background-cover<?php echo $dynamic_swap_mobile;?>" style="background-image: url(<?php echo Media::getAttachmentSrcById($image_id, 'full'); ?>); <?php echo Util::getInlineBackgroundStyles($data); ?>" data-default-image="<?php echo Media::getAttachmentSrcById($image_id, 'full'); ?>"></div>
                    <?php } 
                if ($link_image === 'yes' && !empty($link)) { ?>
                    </a>
                <?php }
                ...
                <?php }
                    if (!empty($mobile_image_id)) { ?> 
                        <div class="hero__image-container-mobile short-hero uk-background-cover<?php echo $dynamic_swap_mobile;?>" style="background-image: url(<?php echo Media::getAttachmentSrcById($mobile_image_id, 'full'); ?>); <?php echo Util::getInlineBackgroundStyles($data); ?>" data-default-image="<?php echo Media::getAttachmentSrcById($mobile_image_id, 'full'); ?>">
                        </div>
                    <?php } else { ?>
                        <div class="hero__image-container-mobile short-hero uk-background-cover<?php echo $dynamic_swap_mobile;?>" style="background-image: url(<?php echo Media::getAttachmentSrcById($mobile_image_id, 'full'); ?>); <?php echo Util::getInlineBackgroundStyles($data); ?>" data-default-image="<?php echo Media::getAttachmentSrcById($image_id, 'full'); ?>">
                        </div>
                    <?php } 
                if ($link_image === 'yes' && !empty($link)) { ?>
                    </a>
    ...
    <?php if ($type === 'product-campaign') { ?>
        <div class="hero__banner-content text-color-<?php echo esc_attr($text_color); ?>">
            <?php if (!empty($subtitle)) {
                printf(
                    '<h1 class="hero__subtitle">%1$s</h1>',
                    nl2br(strip_tags($subtitle, ALLOWED_FORMAT_TAGS))
                );
            } ?>

            <?php if (!empty($content)) { ?>
                <p class="hero__campaign-content"><?php echo strip_tags($content, ALLOWED_FORMAT_TAGS); ?></p>
            <?php } ?>
        </div>
    <?php } ?>

    <?php if ($type === 'product-hero') {
        if (!empty($performance_product)) {
            $logo_variant = ACF::getField('performance-product-logo-variant', $data, 'dark');
            $logo_size = ACF::getField('performance-product-logo-size', $data, 'small');
            $additional_logo = ACF::getField('additional-logo', $data, 'hide');

            do_action('cn/performance-product-banner/output', $performance_product, $logo_variant, $additional_logo, $logo_size);
        }
    } ?>
</section>
